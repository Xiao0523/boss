<template>
  <div>
    <section class="top-box">
      <div class="left">
        <el-form :label-position="labelPosition" :model="fromMenu" label-width="100px">
          <el-form-item label="文章标题：">
            <el-input v-model="fromMenu.title" />
          </el-form-item>
          <el-form-item label="发 布 人：">
            <el-input v-model="fromMenu.issuedBy" />
          </el-form-item>
          <el-form-item label="发布日期：">
            <el-input v-model="fromMenu.date" />
          </el-form-item>
        </el-form>
      </div>
    </section>
    <Tinymce v-model="fromMenu.content" />
    <previewDialog
      :is-show="visible"
      :title="'预览'"
    ...
    >
      <section class="previewContent" v-html="fromMenu.content" />
    </previewDialog>
    <!--封面图上传-->
    <section class="coverimgBox">
      <!--单张封面-->
      <coverCrop
        v-if="fromMenu.layout ==0 || fromMenu.layout ==1"
        :image-url="imageUrl"
        @uploadFileFn="uploadFileFn1"
      />
      <!--多张封面-->
      <coverCrop v-if="fromMenu.layout ==3" :image-url="imageUrl1" @uploadFileFn="uploadFileFn1" />
      <twoCover v-if="fromMenu.layout == 3" :image-url="imageUrl2" @uploadFileFn="uploadFileFn2" />
      <threeCover v-if="fromMenu.layout == 3" :image-url="imageUrl3" @uploadFileFn="uploadFileFn3" />
    </section>
...
export default {
  ...
  computed: {
    // 单图默认回显的封面图
    imageUrl() {
      if (this.fromMenu.bannerPic) {
        const covermaps = this.fromMenu.bannerPic;
        this.bannerPic = covermaps; // 编辑时
        if (covermaps.length) {
    ...
    imageUrl1() {
      if (this.fromMenu.covermaps) {
        const covermaps = this.fromMenu.covermaps;
