<template>
  <div>
    ...
    <el-dialog :visible.sync="isShow" title="文章类目选择" width="30%">
      <el-checkbox :indeterminate="isInderminate" v-model="checkAll" @change="checkAllChange">全选</el-checkbox>
      <el-checkbox-group v-model="choosenTags" class="checkbox-group">
        <el-checkbox
          v-for="item in tags"
          :key="item.id"
          :label="item.id"
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      const index = this.articles.findIndex(item => item.hdId == this.currentArticle.hdId)
      patchArticle({
